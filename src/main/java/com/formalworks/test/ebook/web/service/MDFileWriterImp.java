...
import com.formalworks.test.ebook.web.controller.ParagraphController;
...
class MDFileWriterImp implements MDFileWriter {
	...
	@Autowired
	private ParagraphController paraCon;
	private MakeFullMDContents fullContents;
	 ...
	 * @return
	 */
	public void setFullContents(MakeFullMDContents fullContents) {
	...
	public void write(EBook ebook) throws IOException {
		fullContents = new MakeFullMDContents();
		...
		if (ebook.getFullMDContents() == null || ebook.getFullMDContents().isEmpty()) {
			paraCon = new ParagraphController();
			paraCon.optionApply(ebook);
			ebook.setFullMDContents(fullContents.mergeFullMDContents(ebook));
		}


//		if (ebook.getFullMDContents() == null || ebook.getFullMDContents().isEmpty()) {
//			contents = ebook.getOriginalContents();
//		} else {
//		}
